...
def make_all_summary(summary_df, correction, window_size, threshold_min, threshold_days, dataid, birth, age, gender):
    """
    ...
    if len(summary_df) >= window_size:
        ...
        for i in range(len(summary_df) - window_size + 1):
            ...
            count_above_threshold = (window[wear_time_col_name] >= threshold_min).sum()
            total_wear_time = window[wear_time_col_name].sum()
    ...
    selected_df = selected_df[selected_df[wear_time_col_name] >= threshold_min] # threshold_min 秒以上の日のみを残す
    if (selected_df[wear_time_col_name] >= threshold_min).sum() >= threshold_days:
    ...
    info_df = pd.DataFrame({
        ...
        'Age': [age],
        'BMI': [0.],
        '600分以上の日数': [(selected_df[wear_time_col_name] >= threshold_min).sum()],
        '': [pd.NA]
    })
...
if __name__ == "__main__":
    ...
    make_all_summary(df, "なし", 7, 36000, 4, "data001", "2000/4/1", 30, "F")
